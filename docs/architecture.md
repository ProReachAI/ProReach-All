...
1. Add user authentication and bind every database query to `workspace_id`.
2. Store encryption keys in a managed secret store and define key rotation.
3. Complete provider app review and publish privacy/data-deletion policies.
4. Configure a queue with retry/backoff and dead-letter handling.
5. Add idempotency keys and provider-specific rate-limit budgets.
6. Add webhook signature verification where provider webhooks are enabled.
7. Add content moderation, prohibited-claim checks, and brand facts as grounded inputs.
8. Add analytics ingestion only for approved scopes; minimize retained personal data.
